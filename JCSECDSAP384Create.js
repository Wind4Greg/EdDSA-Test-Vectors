...
console.log(cannon);
writeFile('./output/canonDocJCSECDSAP384.txt', cannon);
...
writeFile('./output/docHashJCSECDSAP384.txt', bytesToHex(docHash));
...
writeFile('./output/proofConfigJCSECDSAP384.json', JSON.stringify(proofConfig, null, 2));
...
writeFile('./output/proofCanonJCSECDSAP384.txt', proofCanon);
...
writeFile('./output/proofHashJCSECDSAP384.txt', bytesToHex(proofHash));

// Combine hashes
let combinedHash = concatBytes(proofHash, docHash); // Hash order different from draft
writeFile('./output/combinedHashJCSECDSAP384.txt', bytesToHex(combinedHash));
...
writeFile('./output/sigHexJCSECDSAP384.txt', bytesToHex(signature.toCompactRawBytes()));
console.log("Computed Signature from private key:");
console.log(base58btc.encode(signature.toCompactRawBytes()));
writeFile('./output/sigBTC58JCSECDSAP384.txt', base58btc.encode(signature.toCompactRawBytes()));
...
writeFile('./output/signedJCSECDSAP384.json', JSON.stringify(signedDocument, null, 2));
