...
console.log(cannon);
writeFile('./output/canonDocDataInt.txt', cannon);
...
writeFile('./output/docHashDataInt.txt', bytesToHex(docHash));
...
writeFile('./output/proofConfigDataInt.json', JSON.stringify(proofConfig, null, 2));
...
writeFile('./output/proofCanonDataInt.txt', proofCanon);
...
writeFile('./output/proofHashDataInt.txt', bytesToHex(proofHash));

// Combine hashes
let combinedHash = concatBytes(proofHash, docHash); // Hash order different from draft
writeFile('./output/combinedHashDataInt.txt', bytesToHex(combinedHash));
...
writeFile('./output/sigHexDataInt.txt', bytesToHex(signature));
console.log("Computed Signature from private key:");
console.log(base58btc.encode(signature));
writeFile('./output/sigBTC58DataInt.txt', base58btc.encode(signature));
...
writeFile('./output/signedDataInt.json', JSON.stringify(signedDocument, null, 2));
