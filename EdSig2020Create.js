/*
    Steps to create a signed verifiable credential in the *Ed25519Signature2020*
    representation.
*/
...
console.log(cannon);
writeFile(baseDir + 'canonDocEdSig.txt', cannon);
...
writeFile(baseDir + 'docHashEdSig.txt', bytesToHex(docHash));
...
writeFile(baseDir + 'proofConfigEdSig.json', JSON.stringify(proofConfig, null, 2));
...
writeFile(baseDir + 'proofCanonEdSig.txt', proofCanon);
...
writeFile(baseDir + 'proofHashEdSig.txt', bytesToHex(proofHash));

// Combine hashes
let combinedHash = concatBytes(proofHash, docHash); // Hash order different from draft
writeFile(baseDir + 'combinedHashEdSig.txt', bytesToHex(combinedHash));
...
writeFile(baseDir + 'sigHexEdSig.txt', bytesToHex(signature));
console.log("Computed Signature from private key:");
console.log(base58btc.encode(signature));
writeFile(baseDir + 'sigBTC58EdSig.txt', base58btc.encode(signature));
...
writeFile(baseDir + 'signedEdSig.json', JSON.stringify(signedDocument, null, 2));
